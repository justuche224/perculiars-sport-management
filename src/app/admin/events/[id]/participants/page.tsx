    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (eventParticipantsData) {
          ...
          setSelectedParticipants(new Set(eventParticipantsData.map((ep) => ep.participant_id)))
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedParticipants.size > 0) {
        const participantsToAdd = Array.from(selectedParticipants).map((participantId) => ({
          event_id: eventId,
      ...
      setError(error instanceof Error ? error.message : "Failed to save participants")
    } finally {
      setIsSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Manage Event Participants</h1>
          <p className="text-gray-600 mt-2">Select participants for this event</p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            {selectedParticipants.size > 0 ? (
                ...
                {Array.from(selectedParticipants).map((participantId) => {
                  ...
                  return (
                    <div key={participantId} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      <div className="flex items-center gap-2">
                        {participant.house && (
                          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: participant.house.color }} />
                        )}
                        <span className="text-sm font-medium">{participant.full_name}</span>
                        <span className="text-sm text-gray-600">({participant.house?.name})</span>
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => handleParticipantToggle(participantId, false)}>
                        Remove
                      </Button>
                    </div>
                  )
              ...
              <p className="text-gray-600 text-center py-8">No participants selected</p>
            )}
          </CardContent>
        </Card>
      </div>

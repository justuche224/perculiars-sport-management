        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="font-medium">{formatDateTime(event.scheduled_time)}</div>
            </div>
            <div>
      ...
      {hasResults ? (
                ...
                {results.slice(0, 3).map((result) => (
                  <div key={result.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg ${getPositionColor(result.position)}`}
                      >
                        {result.position}
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-semibold">{result.participant?.full_name}</div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                All Results ({results.length} of {totalParticipants} participants)
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {results.map((result) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm ${getPositionColor(result.position)}`}
                      >
                        {result.position}
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="font-medium text-sm">{result.participant?.full_name}</div>
                          <div className="text-xs text-gray-600">{result.house?.name}</div>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-medium text-green-600">+{result.points_awarded}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No Results Recorded</h3>
            <p className="text-gray-600 mb-6">Results haven't been recorded for this event yet.</p>
            <Button asChild>
              <Link href={`/admin/results/${id}/record`}>
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
              <div className="text-2xl font-bold text-yellow-600">{event.sport?.points_first}</div>
              <div className="text-sm text-gray-600">1st Place</div>
            </div>
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="text-2xl font-bold text-gray-600">{event.sport?.points_second}</div>
              <div className="text-sm text-gray-600">2nd Place</div>
            </div>
            <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
              <div className="text-2xl font-bold text-amber-600">{event.sport?.points_third}</div>

    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (eventData)
          setEvent({
            ...eventData,
            sport: Array.isArray(eventData.sport)
              ? eventData.sport[0]
              : eventData.sport,
          } as Event);
        if (participantsData) {
          const participantsList = participantsData
            .map((ep) => ep.participant)
            .filter(Boolean)
            .flat() as unknown as Participant[];
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Record Results</h1>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground mb-4">
                Points: 1st ({event.sport?.points_first}pts) | 2nd (
                {event.sport?.points_second}pts) | 3rd (
              ...
              {[1, 2, 3].map((position) => {
                ...
                const participant = winner
                  ? participants.find((p) => p.id === winner.participant_id)
                  : null;
                ...
                return (
                  ...
                  >
                    ...
                    {participant && (
                      <Badge variant="secondary">
                        {getPointsForPosition(position)} points
                      </Badge>
                    )}
                  </div>
                );

    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">Manage participant registrations and house assignments</p>
        </div>
        <Button asChild>
      ...
      {participantsByHouse && Object.keys(participantsByHouse).length > 0 ? (
          ...
          <div key={houseName} className="space-y-4">
            <div className="flex items-center gap-2">
              {houseParticipants[0]?.house && (
                <div className="w-4 h-4 rounded-full" style={{ backgroundColor: houseParticipants[0].house.color }} />
              )}
              <h2 className="text-xl font-semibold text-gray-800">{houseName}</h2>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {participant.guardian_email && (
                        <p className="text-sm text-gray-600">Guardian: {participant.guardian_email}</p>
                      )}
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/admin/participants/${participant.id}/edit`}>Edit</Link>
                        </Button>
                      </div>
                    </div>
                  </CardContent>
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">Get started by registering your first participant.</p>
            <Button asChild>
              <Link href="/admin/participants/new">
                <Plus className="h-4 w-4 mr-2" />
                Add Participant

...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  let { data: profile, error } = await supabase.from("profiles").select("role").eq("id", user.id).single()
  ...
  if (!profile) {
    console.log("[v0] No profile found, creating one for user:", user.email)
    ...
    profile = newProfile
    console.log("[v0] Created profile with role:", profile?.role)
  }

  if (!profile || !["admin", "house_captain"].includes(profile.role)) {

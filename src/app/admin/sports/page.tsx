    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage sports categories and point systems
          </p>
        </div>
        <Button asChild>
      ...
      {sportsByCategory && Object.keys(sportsByCategory).length > 0 ? (
        Object.entries(sportsByCategory).map(
          ([category, categorySports]: any) => (
            <div key={category} className="space-y-4">
              <h2 className="text-xl font-semibold text-foreground capitalize">
                {category} Events
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categorySports.map((sport: any) => (
                  <Card
                    key={sport.id}
                  ...
                  >
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <Badge
                          variant={sport.is_active ? "default" : "secondary"}
                        >
                          {sport.is_active ? "Active" : "Inactive"}
                        </Badge>
                      </div>
                      <CardDescription>
                        Max {sport.max_participants_per_house} per house
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {sport.points_first}/{sport.points_second}/
                            {sport.points_third}
                          </span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground mb-4">
              Get started by creating your first sport category.
            </p>
            <Button asChild>
              <Link href="/admin/sports/new">
                <Plus className="h-4 w-4 mr-2" />

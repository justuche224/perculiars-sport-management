...
export default async function RecordsPage() {
  ...
  const performersByCategory = topPerformers?.reduce((acc, performer) => {
  ...
  const getPositionBadge = (position: number) => {
    ...
    return (
      styles[position as keyof typeof styles] || "bg-gray-300 text-gray-700"
    );
  };
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-4 00 mt-2">
              Outstanding performances and house statistics
            </p>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {house.total_points} total points
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        {performersByCategory &&
          Object.keys(performersByCategory).length > 0 && (
            <Card className="shadow-lg liquid-glass-enhanced">
                    ...
                    <div key={category} className="mb-8 last:mb-0">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {performers.slice(0, 6).map((performer: any) => (
                          <div
                            key={performer.id}
                            className="p-4 liquid-glass-enhanced rounded-lg border"
                          >
                              ...
                              <div className="flex items-center gap-2">
                                {performer.house && (
                                  <div
                                    ...
                                    style={{
                                      backgroundColor: performer.house.color,
                                    }}
                                  />
                                )}
                                <span className="font-semibold text-gray-100">
                                  {performer.participant?.full_name}
                                </span>
                              </div>
                            ...
                            <div className="text-sm text-gray-400">
                              {performer.event?.sport?.name}
                            </div>
                            <div className="flex items-center justify-between mt-2">
                              <span className="text-sm text-gray-400">
                                {performer.house?.name}
                              </span>
                              <Badge variant="secondary">
                                +{performer.points_awarded} pts
                              </Badge>
                            </div>
                          </div>
                        ))}
            ...
            {recentAchievements && recentAchievements.length > 0 ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        {achievement.house && (
                          <div
                            className="w-4 h-4 rounded-full"
                            style={{ backgroundColor: achievement.house.color }}
                          />
                        )}
                        <div>
                          <div className="font-semibold text-gray-900">
                            {achievement.participant?.full_name}
                          </div>
                          <div className="text-sm text-gray-600">
                            {achievement.event?.name} -{" "}
                            {achievement.event?.sport?.name}
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">
                  New achievements will appear here as events are completed
                </p>
              </div>
            )}
          </CardContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">
                  {houseStats?.reduce(
                    (sum, house) => sum + house.totalMedals,
                    0
                  ) || 0}
                </div>
              ...
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">
                  {houseStats?.reduce(
                    (sum, house) => sum + house.total_points,
                    0
                  ) || 0}
                </div>
